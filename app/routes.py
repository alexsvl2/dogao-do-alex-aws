...
def editar_pedido(pedido_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return redirect(url_for('pedidos.historico')) # Redireciona para o histórico após editar
        except Exception as e:
            db.session.rollback()
...
def novo_cliente():
    ...
    return redirect(request.referrer or url_for('pedidos.gerenciar_clientes'))
...
def novo_produto():
    try:
        ...
        flash(f'Erro: {e}', 'danger')
    # Redireciona de volta para a página de onde o usuário veio
    return redirect(request.referrer or url_for('pedidos.gerenciar_produtos'))
