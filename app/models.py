...
# A instância do SQLAlchemy é criada aqui e inicializada no __init__.py
db = SQLAlchemy()

# --- MODELOS FINAIS E UNIFICADOS ---
...
# --- NOVO MODELO PARA AS MESAS ---
class Mesa(db.Model):
    """Modelo para as mesas do estabelecimento."""
    __tablename__ = 'delivery_mesas'
    id = db.Column(db.Integer, primary_key=True)
    numero = db.Column(db.Integer, unique=True, nullable=False)
    # Relação para acessar todos os pedidos de uma mesa
...
class Pedido(db.Model):
    ...
    # Relação com Cliente (pode ser nulo para vendas de mesa, se necessário)
    cliente_id = db.Column(db.Integer, db.ForeignKey('delivery_clientes.id'), nullable=False)
    nome_cliente = db.Column(db.String(100), nullable=False)

    # NOVA RELAÇÃO COM MESA (pode ser nulo se for delivery/balcão)
    mesa_id = db.Column(db.Integer, db.ForeignKey('delivery_mesas.id'), nullable=True)
    ...
    # Novo campo para saber se o pedido da mesa está aberto ou fechado
